...
(define (run-simulation init-board init-player last-move max-depth)
  (define depth 0)
  (define current-board init-board)
  (define prev-move last-move)
...
;;          this node.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(struct mm_node (turn select sub-trees wsc bsc move tplays) #:transparent)
...
(define (build-minimax turn max-depth curr-depth last-move)
  ...
  (define max-num-tries (* 4 branching-fac))
  (define moves-tried '())
  (define sub-trees '())
  (define (terr-counter)
    (let*
...
(define (update tree move-list)
  (cond ((null? move-list)
         ...
         (let* ((next-move (generate-move
                            (mm_node-move tree)
                            (mm_node-turn tree)))
                (attempt (try_move (car next-move) (cdr next-move) (mm_node-turn tree))))
        ...
        (else (begin (let* ((attempt (try_move (car (car move-list)) (cdr (car move-list)) (mm_node-turn tree)))
                            (updated-subtrees
                             (map
                              (lambda (x)
                            ...
                            (bsc (cadr best))
                            (select (cddr best)))
                       (begin (if (= attempt 1) (undo_previous_move) (void))
                              (mm_node (mm_node-turn tree)
                                       select
                                       updated-subtrees
                                       wsc bsc (mm_node-move tree) (add1 (mm_node-tplays tree)))))))))
